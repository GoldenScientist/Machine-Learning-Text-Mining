...
prepare_dtm = function(txt,f=cast_dfm){
	...
	return(test_a %>%
  		count(phrase.ids, word, sort = TRUE) %>% 
#		bind_tf_idf(word, phrase.ids,n)%>%
  		f(phrase.ids, word, n) )
}
...
classifier <- naiveBayes(train_data,train_target)
## does not work
...
library(pROC)
rc = roc(predictor = predict(tmod_nb, 
	newdata = dfmat_matched,type="prob" )[,"TRUE"],
...
bst_emb <- xgboost(data=as.matrix(document_vecs),
			label=train_target,
			max_depth = 20,
            eta = 1, nthread = 3, 
            nrounds = 10,objective = "binary:logistic")
